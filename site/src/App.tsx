...
export function App() {
  ...
  const booking = (className = 'button button-dark') => <a className={`${className} booking-link`} href={villa.contacts.booking} target="_blank" rel="noopener noreferrer"><span className="booking-link-label">{t.booking}<small>Booking.com</small></span><ArrowUpRight size={18} aria-hidden="true" /></a>;
  return <>
    ...
    <main id="main">
      ...
      <section id="villa" className="villa-section container" aria-labelledby="villa-title"><div className="villa-image"><img src="/images/villa.webp" alt={lang==='sr'?'Vila Drina Lux i privatni bazen':'Drina Lux villa and private pool'} width="1600" height="1067" loading="lazy"/><span className="image-tag">DRINA LUX</span><a href="#gallery" className="image-gallery-link" aria-label={t.photos}><ArrowUpRight size={24}/></a></div><div className="villa-copy" data-reveal><p className="eyebrow">{t.villaLabel}</p><h2 id="villa-title">Drina Lux</h2><p className="body-copy">{t.villaText}</p><div className="facts">{[Bed,Bathtub,SwimmingPool,Waves].map((Icon,i)=><div key={i}><Icon size={29} weight="light" aria-hidden="true"/><span>{i<2?<b>{i===0?'3':'1'} </b>:null}{t.facts[i]}</span></div>)}</div>{villa.bookingReview && <a className="booking-proof" href={villa.contacts.booking} target="_blank" rel="noopener noreferrer" aria-label={`${villa.bookingReview.rating} · ${villa.bookingReview.count} ${t.reviewSource} Booking.com`}><strong>{villa.bookingReview.rating}</strong><span aria-hidden="true">·</span><span>{villa.bookingReview.count} {t.reviewSource} <b>Booking.com</b></span><ArrowUpRight size={14} aria-hidden="true"/></a>}<a className="button button-dark" href="#gallery">{t.photos}<ArrowRight size={18} aria-hidden="true"/></a></div></section>
      ...
      <section id="contact" className="booking-section" aria-labelledby="booking-title"><img src="/images/booking-sunset.jpg" alt="" loading="lazy" width="2048" height="1536"/><div className="booking-shade"/><div className="container booking-content"><p className="eyebrow light">{t.bookingLabel}</p><h2 id="booking-title">{t.bookingTitle}</h2><p>{t.bookingText}</p><div className="booking-actions">{booking('button button-cream')}<button className="button button-outline" onClick={()=>setContact(true)}>{t.contact}<ArrowRight size={18} aria-hidden="true"/></button></div></div></section>
    </main>
    <footer className="footer container"><a href="/" className="brand"><span>DRINA LUX</span><small>{t.footer}</small></a><p>Šor, Loznica · {lang==='sr'?'Srbija':'Serbia'}</p><span>© {new Date().getFullYear()} {t.rights}</span><a className="developer-credit" href="https://dun4ev.com" target="_blank" rel="noopener noreferrer">Website by <span>Dun4ev</span></a></footer>
