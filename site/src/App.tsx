...
export function App() {
  ...
  return <>
    ...
    <section className="hero" aria-labelledby="hero-title">
      ...
      <div className="hero-copy container"><p className="eyebrow light">{t.eyebrow}</p><h1 id="hero-title">{t.hero1}<br/><em>{t.hero2}</em></h1><p className="hero-description">{t.heroText}</p><div className="hero-buttons">{booking('button button-cream')}<a className="hero-secondary" href="#villa">{t.explore}<ArrowRight size={18} aria-hidden="true" /></a></div></div>
      <div className="hero-bottom container"><a href="#main"><ArrowDown size={16} aria-hidden="true"/>{t.scroll}</a><span>{t.heroNote}</span></div>
    </section>
    <main id="main">
      <section className="intro container section-space" aria-labelledby="intro-title"><div className="intro-copy"><p className="eyebrow">{t.introLabel}</p><h2 id="intro-title">{t.intro1}<br/><em>{t.intro2}</em></h2><p className="body-copy">{t.introText}</p><a className="text-link" href="#villa">{t.explore}<ArrowRight size={19} aria-hidden="true"/></a></div><figure className="intro-image"><img src="/images/river-sunset.webp" alt={lang==='sr'?'Zalazak sunca iznad Drine':'Sunset over the Drina river'} width="1600" height="1067" loading="lazy"/><figcaption>{t.riverCaption}</figcaption></figure></section>
      <section id="villa" className="villa-section container" aria-labelledby="villa-title"><div className="villa-image"><img src="/images/villa.webp" alt={lang==='sr'?'Vila Drina Lux i privatni bazen':'Drina Lux villa and private pool'} width="1600" height="1067" loading="lazy"/><span className="image-tag">DRINA LUX</span><a href="#gallery" className="image-gallery-link" aria-label={t.photos}><ArrowUpRight size={24}/></a></div><div className="villa-copy"><p className="eyebrow">{t.villaLabel}</p><h2 id="villa-title">Drina Lux</h2><p className="body-copy">{t.villaText}</p><div className="facts">{[Bed,Bathtub,SwimmingPool,Waves].map((Icon,i)=><div key={i}><Icon size={29} weight="light" aria-hidden="true"/><span>{i<2?<b>{i===0?'3':'1'} </b>:null}{t.facts[i]}</span></div>)}</div><a className="button button-dark" href="#gallery">{t.photos}<ArrowRight size={18} aria-hidden="true"/></a></div></section>
      <section id="gallery" className="gallery-section container section-space" aria-labelledby="gallery-title"><div className="section-heading"><div><p className="eyebrow">{t.galleryLabel}</p><h2 id="gallery-title">{t.galleryTitle}</h2></div><p>{t.galleryText}</p></div><Gallery lang={lang} photos={photos}/></section>
      <section className="layout-section" id="layout" aria-labelledby="layout-title"><div className="container layout-grid"><div className="layout-copy"><p className="eyebrow">{t.layoutLabel}</p><h2 id="layout-title">{t.layoutTitle}</h2><p className="body-copy">{t.layoutText}</p><div className="room-list">{villa.rooms.map((room,i)=><div key={i}><span>{room.name[lang]}{i<3?` ${i+1}`:''}</span><span>{room.area}</span></div>)}</div><p className="layout-note">{t.layoutNote}</p></div><button className="floor-plan" onClick={()=>setFloor(true)} aria-label={t.layoutAction}><img src="/images/floor-plan.png" alt={t.layoutAlt} width="1179" height="1009" loading="lazy"/><span>{t.layoutAction}<Plus size={18} aria-hidden="true"/></span></button></div></section>
      <section id="experience" className="experience container section-space" aria-labelledby="experience-title"><p className="eyebrow">{t.experienceLabel}</p><h2 id="experience-title">{t.experienceTitle}</h2><div className="experience-grid">{['pool','bbq','river-sunset'].map((image,i)=><article key={image}><img src={`/images/${image}.webp`} alt={t.experiences[i].text} loading="lazy" width="1000" height="1200"/><h3>{t.experiences[i].title}</h3><p>{t.experiences[i].text}</p></article>)}</div><div className="amenities"><h3>{t.amenitiesTitle}</h3><div className="amenities-grid">{t.amenities.map((label,i)=>{const Icon=amenitiesIcons[i];return <div key={label}><Icon size={28} weight="light" aria-hidden="true"/><span>{label}</span></div>})}</div></div></section>
      <section className="location container" aria-labelledby="location-title"><div className="location-image"><img src="/images/river.webp" alt={lang==='sr'?'Pogled na Drinu sa obale':'View of the Drina from the riverbank'} loading="lazy" width="1600" height="1067"/></div><div className="location-copy"><p className="eyebrow">{t.locationLabel}</p><h2 id="location-title">{t.locationTitle}</h2><p className="body-copy">{t.locationText}</p><a className="text-link" href={villa.map} target="_blank" rel="noopener noreferrer">{t.map}<ArrowUpRight size={18} aria-hidden="true"/></a></div></section>
